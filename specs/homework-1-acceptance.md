...
## Перевірки для приймання ДЗ

| Що перевіряє | Чим перевіряється постійно | Разова перевірка на живій БД |
|---|---|---|
| Лічильники сходяться зі списком знахідок | `client` · «a pill count equals the number of cards it leaves behind» + 11 DB-кейсів у `server/test/pulls-findings.it.test.ts` (зокрема «3 + 4 = 7» і «a re-run supersedes…») | перерахунок обох колонок із `GET /pulls/:id/reviews` → зійшлось: `2 findings · 1 CRITICAL · 1 WARNING · score 53` |
| Нових LLM-викликів немає | статично: жоден файл фічі не імпортує провайдера — `findings-summary.ts` тягне лише `scoreFromFindings` і типи контракту | знімок `agent_runs` / `run_traces` до і після серії читань: `3 → 3`, нових рядків логу про провайдера/промпт/токени — 0 |

Разові скрипти для другої колонки навмисно **не** лежать у репозиторії: постійну
гарантію дають тести, а їх джерело збережене в описі PR (`<details>`), щоб
перевірку можна було повторити без мертвого коду в `scripts/`.
