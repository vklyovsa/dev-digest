...
flowchart LR
  subgraph Studio["Local studio (your machine)"]
    ...
    WEB -->|"REST /repos /pulls /agents /skills /runs …"| API
    API --> PG
  end
...
- **Agents** — four built-in reviewers (General, Security, Performance, Test Quality); create/edit your own (model + system prompt + the skills it loads).
- **Skills** — reusable instruction blocks shared between agents: write one in the browser, or import a `.md` / `.zip` through a preview that never unpacks anything executable.
