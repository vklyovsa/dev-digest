...
export default async function pullsRoutes(appBase: FastifyInstance) {
  ...
  app.get('/repos/:id/pulls', { schema: { params: IdParams } }, async (req): Promise<PrMeta[]> => {
    ...
    // Cost of each PR's LATEST SETTLED run — the list's COST column. Same read
    // pattern as the score above: one IN-query, newest-first, first row per PR
    // wins. Restricted to 'done' so a later failed run (which has no usage to
    // account for) does not hide the last run that actually cost money.
